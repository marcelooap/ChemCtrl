...
/**
 * Cria entrada + estoque + vasilhames (mesmo contrato da tela Entradas).
 * Usado na efetivação de validações de recebimento (embalado/vasilhame).
 */
export async function createEntradaCompleta({ data, produtos = [] }) {
  ...
  const estoqueRecords = (entradaPayload.lotes || []).map((lote) => {
    ...
    return {
      ...
      peso_liquido:
        tipo === "embalado" || tipo === "vasilhame"
          ? lote.peso_liquido || null
          : null,
      quantidade_embalagens:
        tipo === "embalado" ? lote.quantidade_embalagens || null : null,
